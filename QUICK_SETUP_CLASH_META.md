...
> 💡 关于 `filter` 里的 `(?i)`：这是 Go 正则的「忽略大小写」写法，JavaScript 的 RegExp
> 并不支持。MiSub 会自动把它翻译成等效的 JS 正则标志，所以上面的写法可以直接用。
> 另外：如果某个 `filter` 一个节点都没匹配上，该代理组会被自动移除（空的代理组会让
> mihomo 拒绝加载整份配置），引用它的规则会自动改指向兜底组。
