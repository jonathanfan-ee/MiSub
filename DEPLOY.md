...
进入 `misubfu` 项目 → `设置` → `绑定` → `添加`，添加两条：

**第一条** —— KV 命名空间
- 变量名：`MISUB_KV`
- 选择的资源：步骤 1 创建的 `MISUBFU_KV`

**第二条** —— D1 数据库
- 变量名：`MISUB_DB`
- 选择的资源：步骤 2 创建的 `misubfu-database`

> ⚠️ **变量名请手动输入，不要从表格或网页里复制粘贴。**
> 复制时极易带上尾随的空格或制表符，变成 `MISUB_DB␉`。这时面板上显示的名字
> 和正确的看起来一模一样，但代码里 `env.MISUB_DB` 取不到，表现为
> 「明明绑定了却报未绑定 D1」—— 实际踩过一次，排查很费时间。
>
> 现在代码会自动容错这种情况并在日志里提示，但仍建议把名字写干净。

> 变量名必须是 `MISUB_KV` 和 `MISUB_DB` —— 代码里读的就是这两个名字。
> 「选择的资源」才是区分客户与你自己的关键。
> 对照你自己的 `misub-jofanexus` 项目：变量名相同，但资源分别是
> `MISUB_KV` / `misub-database`。
...
> ⚠️ **这一步不能跳过，而且很容易被误判。** 已实测确认，Cloudflare Pages 的两类配置
> 生效方式不一致：
>
> | 配置 | 是否需要重新部署 |
> | --- | --- |
> | 变量和密钥（`ADMIN_PASSWORD` / `COOKIE_SECRET`） | **不需要**，即时生效（改完立刻能登录） |
...
>
> 排查顺序建议：先确认绑定名没带空白字符，再确认改完绑定后重新部署过。
> 这两个原因的表现完全相同，我们两个都踩过。
